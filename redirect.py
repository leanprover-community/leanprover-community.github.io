...
def write_topic_index(stream_name, stream):
    ...
    # stream/<stream-id>-<stream-name>/index.html
    target_url = 'https://leanprover-community.github.io/archive/stream/' + sanitize_stream_new(stream_name, stream['id']) + '/index.html' # / Path(sanitize_stream(stream_name, stream['id'], 'index.html'))

    write_topic_index_header(outfile, stream_name, stream)
    outfile.write('The archive URL format has changed. Please find this page at:\n\n' + '<{0}>'.format(target_url))
    outfile.close()
...
def write_topic(stream_name, stream, topic_name):
    target_url = 'https://leanprover-community.github.io/archive/stream/{1}/topic/{0}.html'.format(sanitize_topic_new(topic_name), sanitize_stream_new(stream_name, stream['id']))
    ...
    o.write('The archive URL format has changed. Please find this page at:<br><br>\n\n' + '<a href="{0}">{0}</a>'.format(target_url))
    o.close()
...
# create a unique sanitized identifier for a topic
def sanitize_topic_new(topic_name):
    return urllib.parse.quote(topic_name, safe='~()*!.\'').replace('.','%2E').replace('%','.')

# create a unique sanitized identifier for a stream
def sanitize_stream_new(stream_name, stream_id):
    return str(stream_id) + '-' + sanitize_new(stream_name)
