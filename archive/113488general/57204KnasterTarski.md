...
<p>pooh favoh jeez ki now fazeeaish eesoo</p>
...
<p><span class="user-mention" data-user-id="110038">@Kevin Buzzard</span> if you're interested</p>
...
<p>it's related to the link I pm'ed you</p>
...
<p>Actually I'm a bit surprised it's not there</p>
...
<p>I'm certain there's something like it somewhere, but maybe it's not stated on complete lattices</p>
...
<p>I thought it's a theorem of complete lattices</p>
...
<p>There is a <code>lfp</code> function in ordinal with a similar proof attached</p>
...
<p>that's the theorem about normal ordinal functions right</p>
...
<p>maybe you can prove your <code>lfp</code> using this :D</p>
...
<p>nvm it isn't a complete lattice</p>
...
<p><code>is_normal.nfp_fp</code> is what I was thinking of</p>
...
<p>Dunno, it's not a complete lattice</p>
...
<p>Maybe that means there's a hidden abstraction?</p>
...
<p>like a sup-continuous monotone function</p>
...
<p>but my function isn't sup-continuous</p>
...
<p>Are you sure? Seems like it follows from the assumptions</p>
...
<p>Anyway, I think this is why it isn't abstracted :)</p>
...
<p>Then you could skip the <code>subtype.val ''</code> part</p>
...
<p>For the big ol <code>complete_lattice</code> proof, I don't think that's the claim you want. Rather, <code>fixed_points f</code> is a complete sublattice of the powerset</p>
...
<p>and then you have that construction generally for all sublattices</p>
...
<p>I'm not aware of the existence of sublattices in Lean?</p>
...
<p>oh, I see, you need that <code>next</code> thing in the middle</p>
...
<p>Hm, it's something like a retract from <code>set A</code> to <code>fixed_points f</code></p>
...
<p>It still strikes me as a generic construction</p>
...
<p><a href="https://github.com/kckennylau/Lean/blob/master/Knaster-Tarski.lean" target="_blank" title="https://github.com/kckennylau/Lean/blob/master/Knaster-Tarski.lean">https://github.com/kckennylau/Lean/blob/master/Knaster-Tarski.lean</a></p>
...
<p><code>Sup (f '' A)</code> is also expressible as an indexed sup, <code>⨆ x ∈ A, f x</code></p>
...
<p><span class="user-mention" data-user-id="110049">@Mario Carneiro</span> so what is the verdict?</p>
...
<p>I prefer aux3 since it is more likely to be used as a lemma</p>
...
<p>do you find ways of abstracting Knaster-Tarski?</p>
...
<p>I wonder if it works in conditionally complete lattices</p>
...
<p><span class="user-mention" data-user-id="110294">@Johannes Hölzl</span> should also weigh in, he's more involved in complete lattice theory than me</p>
...
<p>sup of nonempty bounded sets</p>
...
<p>I think they can do without each other then</p>
...
<p>the least fixed point is from the top element of the original lattice</p>
...
<p>f(x)=x+1 has no fixed points</p>
...
<p>[0,infty) is downward complete right</p>
...
<p>you don’t even need continuity</p>
...
<p>wait, doesn't that imply banach's fp theorem in 1D?</p>
...
<p><a href="https://github.com/leanprover/mathlib/pull/88" target="_blank" title="https://github.com/leanprover/mathlib/pull/88">https://github.com/leanprover/mathlib/pull/88</a></p>
...
<p>There is already <code>order/fixed_points.lean</code> in mathlib. It contains <code>lattice.lfp</code> and <code>lattice.gfp</code> to compute the least and greatest fixed point of a function. For conditionally complete lattices: we could add a subtype instance for intervals on conditionally complete lattices. Then  <span class="katex"><span class="katex-mathml"><math><semantics><mrow><mo>[</mo><mn>0</mn><mo separator="true">,</mo><mn>1</mn><mo>]</mo><mo>:</mo><mrow><mi mathvariant="double-struck">R</mi></mrow></mrow><annotation encoding="application/x-tex">[0, 1] : \mathbb{R}</annotation></semantics></math></span><span aria-hidden="true" class="katex-html"><span class="strut" style="height:0.75em;"></span><span class="strut bottom" style="height:1em;vertical-align:-0.25em;"></span><span class="base"><span class="mopen">[</span><span class="mord mathrm">0</span><span class="mpunct">,</span><span class="mord mathrm">1</span><span class="mclose">]</span><span class="mrel">:</span><span class="mord"><span class="mord mathbb">R</span></span></span></span></span> would be a complete lattice.</p>
...
<p>Your <code>next</code> and <code>previous</code> function can be represented using the <code>gfp</code> and <code>lfp</code>: e.g. <code>gfp (λz, x ⊔ f z) = Sup {z | z ≤ x ⊔ f z}</code>. Then you can use the lfp and gfp properties.</p>
...
<p>so you're just going to close my PR?</p>
...
<p>No, I don't want to close it. I would like to see the fixed point lattice in <code>order/fixed_points.lean</code>. And using the constants and proofs we already have.</p>
...
<p>are you going to edit it or am I going to edit it?</p>
...
<p>Of course, if you want you can edit it! I have a couple more changes: rename <code>aux1..4</code> to something useful and replace <code>is_ord_hom</code> by the assumption <code>monotone f</code>, also going from type class to just assumption.</p>
...
<p>how would you rename <code>aux1..4</code>? <span class="user-mention" data-user-id="110294">@Johannes Hölzl</span></p>
...
<p>something like <code>sup_le_f_sup_of_fixed_points</code>?</p>
...
<p>do you use <code>f</code> in names?</p>
...
<p>Wait -- aren't you and I editing some schemes files with function names like <code>structure_sheaf_of_rings_on_affine_scheme</code> ?</p>
...
<p>my mind replaces <code>f</code> with <code>apply</code> in names</p>
...
<p>yes, and it's always you who type it :P</p>
...
<p><code>definition  structure_sheaf_of_rings_on_affine_scheme (R : Type*) [comm_ring R] : is_sheaf_of_rings (structure_presheaf_of_rings_on_affine_scheme R) :=...</code> :-)</p>
...
<p>well Mario told me I couldn't use <code>tag00EJ</code>...</p>
...
<p>Perhaps <code>tag00EJ</code> is as bad as <code>aux1</code> right? ;-)</p>
