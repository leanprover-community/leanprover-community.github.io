...
<p>I'm happy to do whatever is required, but would like some instructions on how to set up repos with dependencies <code>mathlib &lt;--- repoA &lt;--- repoB</code>, so <code>leanpkg upgrade</code> does what I hope it does. :-)</p>
...
<p>My guess is that the current instruction is: "set <code>lean_version=master</code> in all your leanpkg.toml files, and work on the master branch of <code>repoA</code>"</p>
...
<p>I am still planning on keeping <code>lean-3.4.1</code> branch up to date with master, but I expect that it may drift behind from time to time. Maybe you can consider it the "stable version", but it's really just me being lazy :)</p>
...
<p>Nope, that doesn't work.. Maybe <code>lean_version="nightly"</code>?</p>
...
<p>If you want bleeding edge you should probably stick to the master branch</p>
...
<p>Nope: <code>error: binary package was not provided for 'darwin'</code></p>
...
<p>I still really don't understand the algorithm <code>leanpkg upgrade</code> uses to pick which commit to go to. It seems to be affected by both the setting of <code>lean_version</code>, and perhaps the name of the branch you're currently in in your local repo?</p>
...
<p>Okay, I think the tooling is actually defective, and there's currently no way to have a chain of repos <code>mathlib &lt;--- repoA &lt;--- repoB</code> so that <code>leanpkg upgrade</code> tracks the master branch of mathlib.</p>
...
<p>(it _is_ possible to keep up with mathlib's 3.4.1 branch, by having repoA in a <code>lean-3.4.1</code> branch, and setting <code>lean_version="3.4.1"</code> in all the leanpkg.toml files)</p>
...
<p>I think it's unreasonable to expect someone to continuously sync one branch to another, especially without being written down as part of a process or a checklist. It will certainly be forgotten. If you really want branches synced, it should be automated (e.g. by cron).</p>
...
<p>Numbered versions for mathlib would be fine, but we'd need to switch to a numbering system that is independent of Lean, as we know that Lean versions can get stuck for a long time! Is there some mechanism for leanpkg to understand independent versions for Lean and for mathlib? It seems not.</p>
...
<p>Furthermore mathlib doesn't really have "releases" like a normal software project, it's basically a continuous stream of improvements, so the only reasonable versioning I can see is date-based</p>
...
<p>I agree with version numbers independent of Lean and with date-based version numbers. <a href="https://calver.org/" target="_blank" title="https://calver.org/">CalVer</a> appears to be a new name for the latter.</p>
...
<p>I think we can merge this with plans to precompile mathlib easily enough</p>
...
<p>for example, have mathlib nightlies labeled by date</p>
...
<p>But nightlies represent in-progress work, not released work. You want a release version to indicate some stopping point that describes some meaningful change wrt the last version.</p>
...
<p>Using nightlies as versions is no better than using commit hashes.</p>
...
<p>A formal verification project acts like a software project in many ways, but this is one point of difference, I think.</p>
...
<p>Is <code>set.mm</code> a software library used by others? Or is it more like verified documentation?</p>
...
<p>It is neither. It is a collection of proven theorems, roughly laid out like a math textbook</p>
...
<p>That sounds like documentation rather than software. In which case, I don't see the parallel to mathlib.</p>
...
<p>I'm not sure I understand - mathlib is in the same position, except the documentation needs more work</p>
...
<p>Can I take <code>set.mm</code> and build my own software with it?</p>
...
<p>you can take <a href="http://set.mm" target="_blank" title="http://set.mm">set.mm</a> and build your own theorems with it</p>
...
<p>the curry howard isomorphism is not so relevant here</p>
...
<p>Fair enough. Is that what people do? Or do people use <code>set.mm</code> as a source for the textbook.</p>
...
<p>Of course it can be both at the same time</p>
...
<p>You can read <a href="http://set.mm" target="_blank" title="http://set.mm">set.mm</a> like a book if you want, learn set theory and what have you, or you can import it into your own theorems and prove something nontrivial without going through the basics</p>
...
<p>the same is true of mathlib, although the first goal needs work</p>
...
<p>However, what is there is already done, finished</p>
...
<p>what is not finished is the larger story of how everything (in some area) comes together</p>
...
<p>But things change in mathlib, as they should. If I'm using mathlib as a foundation for my work, I want to (a) be able to point at a version that I used and (b) look at newer versions to figure out how I need to change my work to fit the changes in mathlib.</p>
...
<p>Mario, I think tagging something with a date is not so hard. So how about you "release" such a tag now and then? For example after the merge of a new "feature" (e.g. Scott's category theory) or something like quotient groups, or whatever.</p>
...
<p>I see your point about the computability stuff, and I have been using mathlib as a place to store my project in progress. Arguably that should be a branch, and possibly there will be some reorganization to address this in the future</p>
...
<p>This may not be standard practice for <code>set.mm</code>, but it is for software engineering in the real world, and it's that way for good reasons.</p>
...
<p>However, everything that has been committed is done, and there is no issue with making reference to it if you wanted</p>
...
<p>Mario, it is not about what is possible, but how easy it is for newbies.</p>
...
<p>My point was not to say that you shouldn't include work-in-progress.</p>
...
<p>Johan, I think nightlies address your concern; Sean seems to want more than this</p>
...
<p>Mario, to play the devil's advocate: "What do you even mean with 'work in progress'?"</p>
...
<p>If the commit builds, then it builds.</p>
...
<p>I mean that I wrote 10 theorems, but I plan to prove a hundred more</p>
...
<p>it doesn't make the 10 theorems wrong</p>
...
<p>But once you are done with the 100 theorems. That commit could be tagged with "mathlib-2018-06-20".</p>
...
<p>But theorem names do change, or theorems get reorganized.</p>
...
<p>So, while everything builds, that's not really the end of the story.</p>
...
<p>And then people would see the tag and know: "Hey, that was a commit in which a lot of theorems that belong together were finally all in mathlib"</p>
...
<p>So, I agree with Sean and Kevin and others that certain "not-work-in-progress" commits should get a bit of extra highlighting by <code>git tag</code></p>
...
<p>That seems reasonable, but I would argue: what really makes that commit special that isn't shared by later commits?</p>
...
<p>when you want to use a finished development, you just need to get some commit after it's all in, not the first</p>
...
<p>True. Explain that to your local sysadmin.</p>
...
<p>Mario, it is all about convenience for users.</p>
...
<p>so all that matters is the news, the announcement "project completed"</p>
...
<p><a href="http://set.mm" target="_blank" title="http://set.mm">set.mm</a> has a news/announcements page for this</p>
...
<p>Yes, with a recognizable tag so that sysadmins can easily pull the "latest release" and jot down the release tag in some notebook.</p>
...
<p>which is just a bunch of blurbs marked by date (not even associated specifically to <a href="http://set.mm" target="_blank" title="http://set.mm">set.mm</a> versions)</p>
...
<p>They won't jot down commit hashes.</p>
...
<p>If the sysadmin start updating software, he just wants to go to <a href="https://github.com/leanprover/mathlib/releases" target="_blank" title="https://github.com/leanprover/mathlib/releases">https://github.com/leanprover/mathlib/releases</a> and grab the latest release.</p>
...
<p>He doesn't care about other stuff.</p>
...
<p>Here's what I think a version release should minimally involve:</p>
...
<p>Right and a good measure of whether a commit is "release-worthy" is whether we have anything sensible to mention at point 3 (in a reasonably short message).</p>
...
<p>How do you feel about replacing (3) with a changelog?</p>
...
<p>That <em>is</em> a change log, but it's wrt versions.</p>
...
<p>I'm aware they carry the same information</p>
...
<p>but I would rather have all changes logged in one place</p>
...
<p>Otherwise you have to go check the history every time to write the change log for that version</p>
...
<p>If the changelog is just updated incrementally, then it may as well just be like the news page I described</p>
...
<p>it need not have those big version headers</p>
...
<p>But a change log would make more sense if it described the points between which the changes were made.</p>
...
<p>That's where version numbers come in.</p>
...
<p>I'm arguing for the following: nightlies, plus changelog labeled by dates / nightly version numbers</p>
...
<p>Mario, how often do you expect updates to the changelog?</p>
...
<p>when a project finishes, some breaking change is made, large or small improvements worth note</p>
...
<p>it would basically be a place for advertising new stuff</p>
...
<p>and it easily supports diffs with zero effort, just look at the dates</p>
...
<p>Mario, I hope it is clear that we are not trying to convince you that less-than-daily releases will be a benefit for <em>you</em>.</p>
...
<p>Whether it involves you or not, it might be useful to have a coalition of library developers to select versions of <code>mathlib</code> that we standardize on. Otherwise, if you rely on two independent packages, they may rely on two subtilely different versions of <code>mathlib</code>.</p>
...
<p>A good place for documenting that choice would be <code>mathlib</code> itself</p>
...
<p>It is fully possible that two independent packages can rely on slightly different versions of mathlib, and while this will not always cause an incompatibility (since in many cases you can update mathlib with no upstream changes), it might. But in this case, one or both packages is out of date, and should be updated to master.</p>
...
<p>If you would like an arbitrary standardization point (and nightlies are too fast for you), let's say to prefer commits that are the first of the month</p>
...
<p>But I can't think of any situation as a third party developer to want to update to a version of mathlib that is not the current master</p>
...
<p>i.e. if you say "let's use version X from now on", it should always be the case that X is the current master or nightly at the time of the announcement</p>
