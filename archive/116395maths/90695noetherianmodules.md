...
### Topic: [noetherian modules](https://leanprover-community.github.io/archive/116395maths/90695noetherianmodules.html)
...
<p>What are the next steps for noetherian modules?<br>
...
<p>The map induced map in (3) is injective if <code>M -&gt; N</code> is an inclusion or quotient map.</p>
...
<p>Is this induced map an example of (one side of) a Galois connection? And is it useful to know this?</p>
...
<p>There are actually three maps forming two Galois connections I think</p>
...
<p>Taking image, taking inverse image, and ...?</p>
...
<p>Whatever the right adjoint to the inverse image is</p>
...
<p>The biggest submodule whose inverse image is contained in the original one</p>
...
<p>Right, so you have <code>N' \sub N</code>, and then you'dd take the image of <code>f^{-1} N' \cap M</code>?</p>
...
<p>Scratch that, that's nonsense</p>
...
<p>Start with a submodule of M</p>
...
<p>Call it <code>M'</code>, and put <code>\cap M'</code> in my formula</p>
...
<p>It's those n whose inverse image is a subset of M'. I think</p>
...
<p>You have it for just a map of sets also.</p>
...
<p>It has a forall where the direct image has an exists</p>
...
<p>Yeah, sets and filters also have this strange right adjoint to preimage. I don't know if this is useful anywhere...</p>
...
<p>Actually, now I think it doesn't exist for submodules</p>
...
<p>Because the inverse image of zero might not be zero</p>
...
<p>So the inverse image map doesn't preserve the empty sup</p>
...
<p>So it can't have a right adjoint</p>
...
<p>So ignore everything I said.</p>
...
<p>Direct and inverse image of submodules should be a Galois connection though</p>
...
<p>Right, and I guess this is a general fact about subobjects in algebraic concrete categories...</p>
...
<p>So it would be cool if we could either deduce it from such a general fact, or have a subtype_galois_connection tactic (-;</p>
...
<p>I guess we can start with the complete lattice of subobjects tactic and the span Galois insertion tactic</p>
...
<p>No, I didn't mean that one. I meant the direct/inverse image connection.</p>
...
<p>That one is usually just a follow-your-nose result.</p>
...
<p>You can take the intersection of all the subobjects which contain the given set. Basically get the complete lattice structure from the intersection instead</p>
...
<p>But then in a particular case, you will probably want a more explicit description of the generated subobject.</p>
...
<p>Mario would say that we shouldn't automate this, but just go for it (-;</p>
...
<p>Does the subobject instance tactic work for R-modules? Does it understand that the structure is <code>module R</code>?</p>
...
<p>this instance is already in mathlib</p>
...
<p>I didn't test the tactic, because that is not yet in mathlib</p>
...
<p><code>linear_algebra.subtype_module</code></p>
...
<p>This approach seems like a way to automate this, but it does not go via the galois connection, since that approach brings in the details of the constructive definition of span (here I mean constructive in the mathematician's sense) which are not as uniform</p>
...
<p>Right so there are three definitions of, say, the submodule of M generated by a set S.</p>
...
<p>1 is the intersection of the submodules containing S.</p>
...
<p>If a Moore collection is generated by the closure with respect to a (possibly infinite) family of finite arity operators, then it is an algebraic closure system (ACS), which has still more properties for free. Almost all of the algebraic sub-things are ACSs</p>
...
<p>2 is the set of elements which can be built up from S from the operations of a module, i.e., we define an inductive type of trees which are expressions in the language of R-modules.</p>
...
<p>3 is the one in terms of linear combinations.</p>
...
<p>(2) seems like an inductive definition that is amenable to automation in the same sense as I have suggested</p>
...
<p>1 and 2 are automatable. 3 is not, you have to think of something which is a good "normal form" (roughly) and then prove a special theorem.</p>
...
<p>I guess it probably doesn't matter much which definition we choose or whether the definitions are consistent between different kinds of structures</p>
...
<p>If we want a one-line setup of the collection, the complete lattice, the span operator and the galois connection, it would be best to take one of the uniform versions as the definition, and leave the nonuniform version as a theorem to be proven separately</p>
...
<p>Yeah, I should have looked into them the first time you mentioned them. I like the Examples section in <a href="https://ncatlab.org/nlab/show/Moore+closure" target="_blank" title="https://ncatlab.org/nlab/show/Moore+closure">https://ncatlab.org/nlab/show/Moore+closure</a> :</p>
<blockquote>
...
<p>I see, so it looks a lot more general than subobjects in an algebraic theory.</p>
...
<p>oh, looks like the WP reference is <a href="https://en.wikipedia.org/wiki/Closure_operator" target="_blank" title="https://en.wikipedia.org/wiki/Closure_operator">https://en.wikipedia.org/wiki/Closure_operator</a></p>
...
<p>"finitary closure operator" in that article corresponds to what I called an ACS</p>
...
<p>Does anyone know why the noetherian branch isn't compiling?</p>
...
<p><span class="user-mention" data-user-id="110044">@Chris Hughes</span>, <span class="user-mention" data-user-id="110064">@Kenny Lau</span>, what are the prospects of getting tensor products of commutative rings into mathlib?</p>
...
<p>I wrote <a href="https://github.com/kckennylau/Lean/blob/master/constructive_tensor_product.lean" target="_blank" title="https://github.com/kckennylau/Lean/blob/master/constructive_tensor_product.lean">half the file</a> a month ago</p>
...
<p><code>is_add_group_hom</code> is now in mathlib</p>
...
<p><a href="https://github.com/leanprover/mathlib/commit/afd1c063638d611fda65db7499ccbd7257c90870" target="_blank" title="https://github.com/leanprover/mathlib/commit/afd1c063638d611fda65db7499ccbd7257c90870">https://github.com/leanprover/mathlib/commit/afd1c063638d611fda65db7499ccbd7257c90870</a></p>
...
<p>What is the current state of quotient groups by the way?</p>
...
<p>Kevin's stuff got merged. There's about 100 lines of the basic facts.</p>
...
<p><span class="user-mention" data-user-id="110044">@Chris Hughes</span> can you point me to it?</p>
...
<p>I was thinking of the <code>left_cosets</code> vs <code>group_quotient</code> issue</p>
...
<p>I think we have two competing versions</p>
...
<p>Everything's <code>quotient_group.quotient</code> now.</p>
...
<p>That's good news, but again something to be changed in Kenny's tensor product file</p>
...
<p>but <code>quotient_group</code> includes quotient by non normal subgroups</p>
...
<p><span class="user-mention" data-user-id="110087">@Scott Morrison</span> It's in <code>group_theory/quotient_group</code></p>
...
<p><span class="user-mention" data-user-id="110044">@Chris Hughes</span>, oh, oops, I thought you were talking about tensor products of rings. :-)</p>
...
<p>couldn't we call this <code>group.quotient</code> instead of <code>quotient_group</code>? The later really looks like the quotient is a group</p>
...
<p>I think <code>group.quotient</code> is a better name.</p>
...
<p>Tensor product PR: <a href="https://github.com/leanprover/mathlib/pull/303" target="_blank" title="https://github.com/leanprover/mathlib/pull/303">https://github.com/leanprover/mathlib/pull/303</a></p>
...
<p><span class="user-mention" data-user-id="110087">@Scott Morrison</span> further feature request?</p>
...
<p><span class="user-mention" data-user-id="110031">@Patrick Massot</span> you might be interested</p>
...
<p>Kenny's tensor product is merged! I'm really super excited by the idea of mathlib taking advantage of Kenny's proving power!</p>
...
<p>it's alright Kevin, I may still have things to add there anyway</p>
...
<p>I may prove the dimension theorem, and then I would prove that the dimension of tensor is the product of the dimension</p>
...
<p><span class="user-mention" data-user-id="110038">@Kevin Buzzard</span> was I too fast merging it? Any suggestions?</p>
...
<p>nothing serious. I'll finish my review. I am trying to get into the habit of reviewing mathlib PRs where the mathematics relates to the perfectoid project.</p>
