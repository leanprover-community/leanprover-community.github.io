...
1. It is omitted when `f` is a concrete function/morphism and `x` is a concrete argument,
...
2. It is denoted by the `_apply` suffix when `f` is a concrete function/morphism
  and `x` is a free variable. Many such lemmas are generated by `simps` on a concrete morphism.
3. It is denoted by the `map_` prefix when `f` is a free variable
