...
Some widely used predicates escape this rule. Those are the predicates that are analogous to an atom already suffixed by the naming convention. Here is a non-exhaustive list:
...
* We use `_mono` for `a ≤ b → f a ≤ f b` and `_anti` for `a ≤ b → f a ≤ f b`, so we also use `_mono` for `Monotone f``, `_anti` for `Antitone f`, `_strictMono` for `StrictMono f`, `_strictAnti` for `StrictAnti f`, etc...
