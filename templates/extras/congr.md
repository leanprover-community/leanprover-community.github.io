...
* [`congrm`](https://leanprover-community.github.io/mathlib4_docs/tactics.html#Mathlib.Tactic.congrM) is the tactic-mode equivalent to `congr()`. In addition to antiquotations supported by `congr()`, the term given to `congrm` can also contain `?_` placeholders, which will turn into new proof goals. [`congrm?`](https://leanprover-community.github.io/mathlib4_docs/tactics.html#tacticCongrm?) is an interactive widget that generates `congrm` calls by clicking on subexpressions to turn them into holes.
